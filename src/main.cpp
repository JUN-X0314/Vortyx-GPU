...
int main() {
    ...
    std::cout << "  Phase:   13 (AI/ML Acceleration + Tensor Layer)" << std::endl;
    ...
    vortyx::log(vortyx::LogLevel::Info, "Not implemented yet: hardware GPU tensor kernels (Tensor Core/CUDA/ROCm - none exist in this repository and none are claimed), FP64, autograd/training, quantized kernels, model file formats (ONNX/PyTorch/SafeTensors), cross-device tensor transfer, distributed graph partitioning, real network distributed transport (HTTP/gRPC - the Phase 12 transport is the in-process loopback), load balancing, work stealing, priority scheduling, Advanced/Resource-Aware Scheduling (benchmark and monitoring data deliberately do NOT influence the Scheduler), memory pooling/suballocation, asynchronous compute engine beyond the Phase 6 TaskQueue, remote execution of platform jobs by device agents, real-time device telemetry, HTTP service API over the Phase 14 control plane, external provider adapters (Supabase/Redis/PostgreSQL stores, cloud queues), billing/marketplace, multi-region clusters.");
